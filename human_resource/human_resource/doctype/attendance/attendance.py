...
class Attendance(Document):
	...
	def add_value_work_hours_and_late_hours(self, is_validate=False):
		...
		working_hours = round(time_diff_in_hours(check_out, check_in),2)
		...
		if (int(settings.late_entry_grace_period) == 0 and int(settings.early_exit_grace_period) == 0):
			...
			check_in = get_time_str(to_timedelta(self.check_in) - timedelta(minutes=late_entry_grace_period))
		elif (int(settings.late_entry_grace_period) == 0 and int(settings.early_exit_grace_period) > 0):
			check_out = get_time_str(to_timedelta(self.check_out) + timedelta(minutes=early_exit_grace_period))
		else:
			check_in = get_time_str(to_timedelta(self.check_in) - timedelta(minutes=late_entry_grace_period))
			check_out = get_time_str(to_timedelta(self.check_out) + timedelta(minutes=0))
